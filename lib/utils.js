...
exports.initializeFormData = initializeFormData;
exports.nullifyEmptyRequiredFields = nullifyEmptyRequiredFields;
...
exports.getNotSpecifiedOption = getNotSpecifiedOption;
...
function initializeFormData(schema, formData) {
  ...
  if (schema && formData && (typeof formData === "undefined" ? "undefined" : _typeof(formData)) === "object") {
    var keys = Object.keys(formData);
    var requiredFields = schema.required;
    for (var i = 0; i < keys.length; i++) {
      var formDataPropertyName = keys[i];
      var formDataPropertyValue = formData[formDataPropertyName];
      var formDataPropertyRequired = requiredFields ? requiredFields.indexOf(formDataPropertyName) > -1 : false;
      var formDataPropertyType = schema.properties[formDataPropertyName] ? schema.properties[formDataPropertyName].type : undefined;
      var formDataPropertyFormat = schema.properties[formDataPropertyName] ? schema.properties[formDataPropertyName].format : undefined;
...
function nullifyEmptyRequiredFields(schema, uiSchema, formData) {
  if (schema && formData && (typeof formData === "undefined" ? "undefined" : _typeof(formData)) === "object") {
    var keys = Object.keys(formData);
    var requiredFields = schema.required;
    for (var i = 0; i < keys.length; i++) {
      ...
      // Otherwise, if this is an array...
        ...
        else if (formDataPropertyType === "string" && ["date", "date-time"].includes(formDataPropertyFormat) && formDataPropertyValue && formDataPropertyValue.substring(0, Math.min(formData[formDataPropertyName].length, 10)) === "0000-01-01") {
            formData[formDataPropertyName] = null;
          }
          // Otherwise, if this is a string, and it's currently empty, set it to null...
            ...
            // Otherwise if this is a number, and it's currently empty, set it to null...
              ...
              // it to the empty string, which corresponds to (Not Specified)...
                  ...
                  var isConsentField = false;
                  var formDataPropertyUiSchema = uiSchema ? uiSchema[formDataPropertyName] : undefined;
                  ...
                  if (formDataPropertyUiSchema) {
                    var _getUiOptions = getUiOptions(formDataPropertyUiSchema),
                  ...
                  if (!isConsentField && formDataPropertyValue === "" || isConsentField && !formDataPropertyValue) {
                    formData[formDataPropertyName] = null;
                  }
                }
    }
...
function getNotSpecifiedOption() {
  return {
    value: "",
    label: "(Not Specified)"
  };
